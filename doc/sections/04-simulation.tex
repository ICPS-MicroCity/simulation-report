...
Instead, \textit{shortest queue in range redirection} assures that the next attraction will be inside a certain range, reducing the route to walk.
...
This is probably due to the physical distribution of visitors, that are much more spread in a random fashion.
...
However, considering the \textit{shortest queue in range redirection}, increasing waiting times also mean that visitors spend more time in queues and less time walking in order to reach the attractions.
Keeping in mind that the number of satisfaction is higher for this policy, this can be considered a positive factor, as this result perfectly fits with the policy's aim.
So, the correct trend to evaluate is the waiting time per attraction one, which is displayed in the plot~\ref{plot:waiting-time-per-attraction}.
